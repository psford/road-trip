...
- **Native SwiftUI app (`ios-swift/RoadTrip/`)** — a separate, ground-up native iOS client (NOT the Capacitor shell above; the two iOS apps coexist). SwiftUI + MapKit, GRDB/SQLite local store (migrations in `RoadTrip/Storage/Migrator.swift`), background `URLSession` resilient uploads, Keychain-backed per-trip tokens. Talks to the same .NET API + Contracts below. Project is **XcodeGen-generated from `ios-swift/RoadTrip/project.yml`** — never hand-edit `RoadTrip.xcodeproj`; `project.yml` excludes `**/*.md` so per-directory `CLAUDE.md` docs don't collide as app-bundle resources. MVP works on-device (2026-06-19); UI polish round 2 (curved route, soft-archive/Archived view, camera capture, floating top bar) in PR #110. Domain docs live in per-directory `CLAUDE.md` files under `ios-swift/RoadTrip/RoadTrip/{Storage,Networking,Photos,Views/Trips}/`; see also `docs/native-ios-handoff.md`.
...
- Native SwiftUI app -- from `ios-swift/RoadTrip`: `xcodegen generate` after adding/removing files, then `xcodebuild -project RoadTrip.xcodeproj -scheme RoadTrip -destination 'platform=iOS Simulator,name=iPhone 17' build` (swap `build` → `test` for the suite). **iPhone 17 is the installed simulator** — iPhone 15 referenced in older plan/docs is not installed. Tests: `RoadTripTests` (unit) + `RoadTripUITests` (UI). The `testCreateTripFlow`/`testImportTripFlow`/`testImportInvalidTokenShowsError` UI tests need a live local API and fail with "Couldn't reach the server" offline — that is environmental, not a regression.
...
## Git Flow (road-trip specifics)

The generic `develop → main` branching, forbidden-operation, and pre-commit rules come from the shared **git-flow-develop-main** fragment above. road-trip adds/overrides:

### Merge strategy
All merges use **"Create a merge commit"** (regular merge), for both feature → develop and develop → main. Squash-merge is reserved for catastrophic-error recovery only (Patrick's directive 2026-05-12); rebase-and-merge is not used. The `git-flow-guard` CI step in `roadtrip-ci.yml` fails on push to `main` if the latest commit has <2 parents (a squash). Squash commands are also blocked at the tool layer by `~/.claude/hooks/no-squash-merge.sh`. Background: `docs/issues/2026-05-09-rca-git-flow-catastrophe.md`.

### Who can do what (Claude vs Patrick) — authoritative, updated 2026-05-24
| Action | Claude | Patrick |
|--------|--------|---------|
| Direct commit to develop (small fixes) | yes, without asking | — |
| Open feature branch + PR -> develop | yes, without asking | — |
| Merge a feature PR -> develop (regular merge commit) | yes, without asking | — |
| Commit/push to feature branches | yes, without asking | — |
| Open develop -> main PR | yes, without asking | — |
| Merge develop -> main | NEVER | Patrick alone, via GitHub/VS Code UI |
| Run `deploy.yml` | NEVER | Patrick triggers (+ cap sync as needed) |
| Run `npx cap sync ios` / Xcode build | NEVER | Patrick runs locally |

Develop and feature branches are Claude's working space — no approval gate. Main is protected: every change to main passes through Patrick at the merge step. This 2026-05-24 policy replaced the earlier "ask first" model (which over-pinged on routine develop merges) and supersedes any older "merge after asking" wording.
